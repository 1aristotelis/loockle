...
const WalletProviderPopUp = () => {
  const { setWallet, wallet, authenticate } = useBitcoin()
  const [seedInputScreen, setSeedInputScreen] = useState(false)
  const { relayXAuthenticate, relayXAuthenticated, relayXPaymail, relayXLogout } = useRelayX()
  const { twetchAuthenticate, twetchAuthenticated, twetchPaymail, twetchLogout } = useTwetch()
  ...
  return (
    <>
          {!seedInputScreen ? (<div className="flex-col p-5 sm:rounded-lg bg-base-100">
            <p className="text-2xl text-center font-bold ">
              Select Wallet
            </p>
            <button
              onClick={handleRelayXAuth}
              className={`bg-base-200 flex w-full mt-8 h-[52px] rounded-full py-2.5 px-5  items-center text-center justify-center font-semibold ${wallet === "relayx" ? "border-2 border-primary" : "border-none"}`}
            >
              <svg
            ...
            <button
              ...
              className={`opacity-50 bg-base-200 flex w-full mt-8 h-[52px] rounded-full py-2.5 px-5 items-center text-center justify-center font-semibold ${wallet === "twetch" ? "border-2 border-primary" : "border-none"}`}
            >
              <svg
                viewBox="0 0 102 110"
                fill="none"
            ...
            <button
              ...
              className={`opacity-50 bg-base-200 flex w-full mt-8 h-[52px] rounded-full py-2.5 px-5 items-center text-center justify-center font-semibold ${wallet === "local" ? "border-2 border-primary" : "border-none"}`}
            >
              <svg xmlns="http://www.w3.org/2000/svg" enableBackground="new 0 0 2499.6 2500" viewBox="0 0 2499.6 2500" className="h-6 w-6">
              ...
              <div className="flex flex-col items-start">
                <p className=" ml-2.5 ">Seed Phrase</p>
            ...
            <div className="flex flex-col p-5 rounded-lg bg-base-100">
              ...
              <div className="-mt-7 flex flex-col w-full">
                ...
                <textarea placeholder="Enter 12 words seed phrase here..." rows={4} value={inputSeedPhrase} onChange={handleChangeSeedPhrase} className='mt-4 w-full textarea textarea-primary'/>
                <div className="flex justify-center mt-4">
                    <button type="button" onClick={handleSeedAuth} className="btn btn-primary">Authenticate</button>
                </div>
              </div>
            </div>
          )}
